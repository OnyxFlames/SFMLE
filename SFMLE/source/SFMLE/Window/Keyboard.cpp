...
namespace sfe
{
	namespace
	{
		static std::unordered_map<std::string, sf::Keyboard::Key> BuildKeyNameTable()
		{
			std::unordered_map<std::string, sf::Keyboard::Key> table;

			for (size_t i = 0; i < sf::Keyboard::Key::KeyCount; ++i) table[sfe::Keyboard::ToString(static_cast<sf::Keyboard::Key>(i))] = static_cast<sf::Keyboard::Key>(i);
	...
	sf::Keyboard::Key Keyboard::FromString(const std::string& key)
	{
